...
class DataValidation:
    ...
    def validate_number_of_columns(self, dataframe:pd.DataFrame)->bool:
        try:
            ...
            logging.info(f"Req no. of col:{number_of_columns}")
            logging.info(f"DataFrame has no. of col:{len(dataframe.columns)}")
    ...
    def validate_numerical_columns(self, dataframe:pd.DataFrame)->bool:
        try:
            ...
            logging.info(f"Req numerical col: {numerical_columns}")

            dataframe_numerical_columns= len(dataframe.select_dtypes(include=["int64","float64"]).columns)
            logging.info(f"Dataframe numerical col: {dataframe_numerical_columns}")

            if numerical_columns== dataframe_numerical_columns:
                return True
            return False
